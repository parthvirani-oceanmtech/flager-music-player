...
class PlaylistNotifier with ChangeNotifier {
  ...
  List<AudioModel> _currentSongModelList = [];
  ...
  void songsPlaylist(int playlistId) async {
    try {
      _currentSongModelList = await _audioQuery.querySongs(
        filter: MediaFilter.forPlaylists(
          playlistSortType: PlaylistSortType.DATE_ADDED,
          orderType: OrderType.ASC_OR_SMALLER,
          toQuery: {
            MediaColumns.Playlist.ID: [playlistId.toString()],
          },
        ),
      );
      notifyListeners();
  ...
  List<AudioModel> get getCurrentSongModelPlaylist => _currentSongModelList;
}
